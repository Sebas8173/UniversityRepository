...
function ReservationList() {
  ...
  const getClientName = (clientId) => {
    ...
    let client = clients.find(c => {
      ...
      const cIdNum = Number.parseInt(c.id);
      const clientIdNum = Number.parseInt(clientId);
      return !Number.isNaN(cIdNum) && !Number.isNaN(clientIdNum) && cIdNum === clientIdNum;
    });
    ...
    if (!client) {
      client = clients.find(c => {
        ...
        const cIdClientNum = Number.parseInt(c.id_client);
        const clientIdNum = Number.parseInt(clientId);
        return !Number.isNaN(cIdClientNum) && !Number.isNaN(clientIdNum) && cIdClientNum === clientIdNum;
      });
    }
  ...
  const getMenuName = (menuId) => {
    ...
    const menu = menus.find(m => {
      ...
      return m.id === menuId || m.id.toString() === menuId.toString() ||
             (!Number.isNaN(Number.parseInt(m.id)) && !Number.isNaN(Number.parseInt(menuId)) && 
              Number.parseInt(m.id) === Number.parseInt(menuId));
    });
