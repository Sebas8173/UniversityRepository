...
function EventForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        await eventsAPI.update({ ...eventData, id: Number.parseInt(id) });
        setSuccess('Evento actualizado exitosamente');
      } else {
        await eventsAPI.create({ ...eventData, id: Number.parseInt(event.id) });
  ...
  const safeEventId = Number.isNaN(event.id) ? '' : event.id;
