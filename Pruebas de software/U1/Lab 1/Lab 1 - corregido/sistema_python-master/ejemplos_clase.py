...
def excepciones(nombre_archivo):
    ...
    # Ejemplo: manejo específico de ZeroDivisionError
    try:
        division = 3 / 0
    except ZeroDivisionError:
        # En lugar de solo notificar, establecemos un valor por defecto y
        # mostramos información al usuario para que la excepción haga algo útil.
        print('Error: división por cero. Se asigna valor por defecto 0.0')
        division = 0.0
    except Exception as e:
        # Captura cualquier otra excepción inesperada
        print('Error inesperado durante la división:', e)
        division = None
    else:
        print('División ejecutada correctamente:', division)
    ...
    # Manejo de apertura de archivo con acciones útiles
    try:
        with open(nombre_archivo, 'r', encoding='utf-8') as fi:
            contenido = fi.read()
            print(f"Se pudo abrir '{nombre_archivo}', contenido:\n{contenido}")
    except FileNotFoundError:
        # Si el archivo no existe, lo creamos con contenido por defecto
        print('No se encuentra el archivo', nombre_archivo, '- Creándolo con contenido por defecto.')
        try:
            with open(nombre_archivo, 'w', encoding='utf-8') as fi:
                fi.write('Archivo creado automaticamente por el programa.\n')
            with open(nombre_archivo, 'r', encoding='utf-8') as fi:
                contenido = fi.read()
                print(f"Contenido nuevo de '{nombre_archivo}':\n{contenido}")
        except Exception as e:
            print('No se pudo crear el archivo:', e)
    except PermissionError:
        print('No tiene permisos para abrir', nombre_archivo)
    except Exception as e:
        print('Error inesperado al intentar abrir el archivo:', e)

    # Finalmente, cerramos el archivo si fue abierto exitosamente
    if 'fi' in locals():
        fi.close()
...
def existencia_archivo(nombre_archivo):
    ...
    if os.access('inove.jpg', os.F_OK) is False:
        print('No se pudo acceder a inove.jpg')
...
def directorios(nombre_archivo):
    ...
    # Si el archivo no fue encontrado por nombre ni por path
    # significa que no existe en el sistema
    else:
